...
def bio_coverage(biosynth_list,biosynth_db, coverage_cutoff):

    dict_bgc={}

    input_handle = open(str(biosynth_db), "r") #or die "can't open database"
    ...
    filtered = open (str(biosynth_list), "r")
    first_line=True
    ...
    for key in sorted_keys:
        if dict_bgc[key].coverage > coverage_cutoff:
            output.write (">%s\t%s\n%s\n"
